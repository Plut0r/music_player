...
function Play(props) {
  ...
  return (
    <div className="w-full h-20 play fixed left-0 right-0 bottom-0 flex items-center justify-between py-10 pl-5 lg:pl-20 pr-10">
      <audio
        src={props.song[props.currentSongIndex].audio}
        ref={props.audioEl}
      />
      <div className="h-[49px] w-48 flex gap-3">
        <div className="rounded-xl">
          <img
            className="w-[49px] h-[49px] rounded-xl object-cover"
            src={props.song[props.currentSongIndex].image}
            alt="user"
          />
        </div>
        <div className="text-white flex flex-col mt-2">
          <h4 className="text-sm font-bold">
            {props.song[props.currentSongIndex].title}
          </h4>
          <p className="text-white/50 font-bold text-xs">
            {props.song[props.currentSongIndex].artist}
          </p>
        </div>
      </div>
      <div className="flex items-center gap-2 lg:hidden">
        <div onClick={() => props.setIsplaying(!props.isPlaying)}>
          {props.isPlaying ? (
            ...
            <div className="text-white text-xs w-6 h-6 rounded-full border border-white flex items-center justify-center">||</div>
          )}
        </div>
      ...
      <div className="hidden lg:flex flex-col items-center">
        <div className="flex items-center gap-5 cursor-pointer">
          ...
          <div onClick={() => props.skipSong(false)}>
          ...
          <div onClick={() => props.setIsplaying(!props.isPlaying)}>
            {props.isPlaying ? (
              ...
              <div className="text-white text-xs w-6 h-6 rounded-full border border-white flex items-center justify-center">
                ||
              </div>
            )}
          </div>
